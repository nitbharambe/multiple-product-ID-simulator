...
class Orderbook(object):
    ...
    def process_order_aon(self, order):
        '''Check for a trade (match); if so call _match_trade, otherwise modify book(s).'''
        self.traded = False
        self.add_order_to_history(order)
        if order['type'] == OType.ADD:
            if order['side'] == Side.BID:
                possible_prices = self._ask_book_prices[order['price'] >= self._ask_book_prices]
                aon_flag = False
                for possible_price in possible_prices:
                    for book_order in self._ask_book[possible_price]['orders']:
                        if book_order['quantity'] == order['quantity']:
                            ...
                            break
                if aon_flag:
                    self._match_trade_aon(order)
                else:
                    self.add_order_to_book(order)
            else:  # order['side'] == 'sell'
                possible_prices = self._bid_book_prices[order['price'] <= self._bid_book_prices]
                aon_flag = False
                for possible_price in possible_prices:
    ...
    def _match_trade_aon(self, order):
        '''Match orders to generate trades, update books.'''
        self.traded = True
        self.confirm_trade_collector.clear()
        if order['side'] == Side.BID:
