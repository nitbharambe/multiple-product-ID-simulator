...
class Orderbook(object):
    ...
    def _confirm_trade(self, timestamp, order_side, order_quantity, order_id, order_price, trader_id,
                       incoming_trader_id, incoming_order_id):
        ...
        self.confirm_trade_collector.append([{'timestamp': timestamp, 'trader': trader_id, 'order_id': order_id,
                                             ...
                                             {'timestamp': timestamp, 'trader': incoming_trader_id,
                                              'order_id': incoming_order_id,
                                              'quantity': order_quantity,
                                              'side': set(Side).difference([order_side]).pop(), 'price': order_price}
                                             ])
    ...
    def process_order(self, order):
        ...
        if order['type'] == OType.ADD:
            ...
            if order['order_id'] not in self._lookup[order['trader_id']].keys():
                print("  ")
    ...
    def process_order_new(self, order):
        '''Check for a trade (match); if so call _match_trade, otherwise modify book(s).'''
        self.traded = False
        self.add_order_to_history(order)
        aon_flag = False
        non_aon_ids_present = False
        if order['type'] == OType.ADD:
            if order['side'] == Side.BID:
                possible_prices = [price for price in self._ask_book_prices if price <= order['price']]
                for possible_price in possible_prices:
                    for book_order in self._ask_book[possible_price]['orders']:
                        if not self._ask_book[possible_price]['orders'][book_order]['trader_id'] in self.aon_traders:
                            non_aon_ids_present = True
                        if self._ask_book[possible_price]['orders'][book_order]['quantity'] >= order['quantity']:
                            aon_flag = True
                if (order['trader_id'] in self.aon_traders and aon_flag) or (
                        not order['trader_id'] in self.aon_traders and non_aon_ids_present):
                    self._match_trade(order)
                else:
                    self.add_order_to_book(order)
            else:  # order['side'] == 'sell'
                possible_prices = [price for price in self._bid_book_prices if price >= order['price']]
                for possible_price in possible_prices:
                    for book_order in self._bid_book[possible_price]['orders']:
                        if not self._bid_book[possible_price]['orders'][book_order]['trader_id'] in self.aon_traders:
                            non_aon_ids_present = True
                        if self._bid_book[possible_price]['orders'][book_order]['quantity'] >= order['quantity']:
                            aon_flag = True
                if (order['trader_id'] in self.aon_traders and aon_flag) or (
                        not order['trader_id'] in self.aon_traders and non_aon_ids_present):
                    self._match_trade(order)
                    ...
                    if order['price'] >= price:
                        ...
                        if remainder >= book_order['quantity']:
                            self._confirm_trade(order['timestamp'], book_order['side'], book_order['quantity'],
                                                book_order['order_id'],
                                                book_order['price'], book_order['trader_id'], order['trader_id'],
                                                order['order_id'])
    ...
    def _match_trade_new(self, order):
        '''Match orders to generate trades, update books.'''
        ...
        if order['side'] == Side.BID:
            ...
            while remainder > 0:
                if book_prices:
                    price = book_prices[skip_counter]
                    if order['price'] >= price:
                        ex_id = book[price]['ex_ids'][0]
                        book_order = book[price]['orders'][ex_id]
                        if remainder >= book_order['quantity']:
                            ...
                            self._remove_order(book_order['side'], book_order['price'], ex_id)
                            remainder -= book_order['quantity']
                        elif book_order['trader_id'] not in self.aon_traders:
                            self._confirm_trade(order['timestamp'], book_order['side'], remainder,
                                                book_order['order_id'],
                                                book_order['price'], book_order['trader_id'], order['trader_id'],
                                                order['order_id'])
                            ...
                            self._modify_order(book_order['side'], remainder, ex_id, book_order['price'])
                            break
                        elif book_order['trader_id'] in self.aon_traders:
                            skip_counter += 1
                    else:
                        order['quantity'] = remainder
                        self.add_order_to_book(order)
    ...
    def report_top_of_book(self, now_time):
        ...
        # TODO Find alternative for vwap
        vwap_buy = sum(i * self._bid_book[i]["size"] for i in self._bid_book.keys()) / sum(
            self._bid_book[i]["size"] for i in self._bid_book.keys())
        vwap_ask = sum(i * self._ask_book[i]["size"] for i in self._ask_book.keys()) / sum(
            self._ask_book[i]["size"] for i in self._ask_book.keys())
        tob = {'timestamp': now_time, 'best_bid': best_bid_price, 'best_ask': best_ask_price, 'bid_size': best_bid_size,
               'ask_size': best_ask_size, "vwap_buy": vwap_buy, "vwap_ask": vwap_ask}
        self._sip_collector.append(tob)
        return tob

    def report_top_of_book_new(self, now_time):
        '''Update the top-of-book prices and sizes'''
        break_out_flag = False
        skip_counter = -1
        for best_bid_price in self._bid_book_prices[::-1]:
            for book_order in self._bid_book[best_bid_price]['orders']:
                if self._bid_book[best_bid_price]['orders'][book_order]['trader_id'] in self.aon_traders:
                    continue
                else:
                    break_out_flag = True
                    break
            if break_out_flag:
                break
        #best_bid_price = self._bid_book_prices[skip_counter]
        break_out_flag = False
        skip_counter = 0
        for best_ask_price in self._ask_book_prices:
            for book_order in self._ask_book[best_ask_price]['orders']:
                if self._ask_book[best_ask_price]['orders'][book_order]['trader_id'] in self.aon_traders:
                    continue
                    #skip_counter += 1
                else:
                    break_out_flag = True
            ...
            if break_out_flag:
                break
        #best_ask_price = self._ask_book_prices[skip_counter]
        best_bid_size = self._bid_book[best_bid_price]['size']
        best_ask_size = self._ask_book[best_ask_price]['size']
        vwap_buy = sum(i * self._bid_book[i]["size"] for i in self._bid_book.keys()) / sum(
            self._bid_book[i]["size"] for i in self._bid_book.keys())
